...
class Worms:
    ...
    def set_takeoff_angle(self):
        ...
        self.shot_vector = QPoint(np.floor(18*np.cos(self.takeoff_angle)), np.floor(18*np.sin(self.takeoff_angle)))
        self.pull = 0
        self.travel_counter = 0
        print(self.shot_vector)
    ...
    def animation(self):
        ...
        #print("px:",px,",x: ",x, " - py:", py, " y:", y,"py-y=",py-y,"x-px=",x-px)
        #print(np.arctan((py - y) / (x - px)))
        #print(self.takeoff_angle)
        painter.drawPixmap(QPoint(0, 0), QPixmap.fromImage(self.bulletImg))

        #Ballistics:
        #direction of shot + magnitude without gravitational pull
        ...
        first_stage = QPoint(self.bulletPos.x() +self.shot_vector.x(),
        ...
        #pauses pull and bullet travel
        if self.frame_count == self.travel_rate:
            self.frame_count = -1
            self.bulletPos = first_stage
            print("dink!")
